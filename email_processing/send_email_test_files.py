...
def build_message(recipient: str, sender: str, attachments: Optional[List[Dict]] = None) -> dict:
    account = "MBB02" # Force MBB02 for testing credit card logic
    recon_date = datetime.utcnow().strftime("%d/%m/%Y")
    amount = "263,737.84" # Arbitrary amount
...
def main():
    ...
    if jompay_path.exists():
        content_bytes = base64.b64encode(jompay_path.read_bytes()).decode("ascii")
        attachments.append(
            {
                "@odata.type": "#microsoft.graph.fileAttachment",
                "name": "daily_transaction_report.xlsx",
                "contentType": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                "contentBytes": content_bytes,
            }
        )
        print(f"Added attachment: {jompay_path.name}")
    else:
        print(f"WARNING: JomPAY file not found at {jompay_path}")

    # 2. Credit Card File
    cc_path = ROOT_DIR / "test_dataset" / "dbkkdemodataset" / "027007700511_T41_20251111.csv"
    if cc_path.exists():
        content_bytes = base64.b64encode(cc_path.read_bytes()).decode("ascii")
        attachments.append(
            {
                "@odata.type": "#microsoft.graph.fileAttachment",
                "name": "CT_20251111.csv", # Renaming slightly to test preservation
                "contentType": "text/csv",
                "contentBytes": content_bytes,
            }
        )
        print(f"Added attachment: {cc_path.name}")
    else:
        print(f"WARNING: Credit Card file not found at {cc_path}")
    ...
    if resp.status_code in (200, 202):
        print(f"OK. Test mail queued.")
        print("Now wait for the email processor to pick it up.")
    else:
